...
function ExpenseListPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: () => expenseService.getExpenseList({ 
      page, 
      perPage: 10,
      ...filters 
    }),
  ...
  const handleExport = () => {
    expenseService.exportExpenses(filters);
  };
      ...
      <div className="bg-white shadow sm:rounded-lg p-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center space-x-2">
            <h2 className="text-lg font-medium text-gray-900">Expense Transactions</h2>
            <button
            ...
            <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
              <div>
                <p className="text-sm text-gray-700">
                  ...
                  <span className="font-medium">{data.total}</span> total expenses
                </p>
              </div>
              <div className="flex items-center space-x-2">
                <button
            ...
            <dl className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <dd className="text-sm font-medium">{selectedExpense.transactionNumber}</dd>
                </div>
                <div>
              ...
              <div>
                ...
                <dd className="text-sm">{selectedExpense.description}</dd>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <dd className="text-sm">{selectedExpense.paymentMethod}</dd>
                </div>
                <div>
                  <dt className="text-sm text-gray-500">Reference</dt>
                  <dd className="text-sm">{selectedExpense.reference || '-'}</dd>
                </div>
              </div>
              <div>
                <dt className="text-sm text-gray-500">Account</dt>
                <dd className="text-sm">
                  {selectedExpense.account?.code} - {selectedExpense.account?.name}
                </dd>
              </div>
              {selectedExpense.notes && (
                <div>
                  <dt className="text-sm text-gray-500">Notes</dt>
                  <dd className="text-sm bg-gray-50 p-2 rounded">{selectedExpense.notes}</dd>
                </div>
              )}
              <div className="border-t pt-4">
                <dt className="text-sm text-gray-500">Status</dt>
                <dd className="mt-1">{getStatusBadge(selectedExpense.status)}</dd>
              </div>
            </dl>
          </div>
        </div>
...
function AddExpensePage() {
  ...
  const mutation = useMutation({
    mutationFn: (data) => expenseService.recordExpense(data),
    onSuccess: () => {
  ...
  return (
    <div className="bg-white shadow sm:rounded-lg">
      <div className="px-4 py-5 sm:p-6">
        <h2 className="text-lg font-medium text-gray-900 mb-4">Record New Expense</h2>
        <AddExpense 
          onSubmit={onSubmit} 
...
function ExpenseAnalytics() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: () => expenseService.getExpenseAnalytics({ period }),
  });
  ...
  if (error) {
    return (
      <div className="bg-white shadow sm:rounded-lg p-6 text-center text-red-600">
  ...
  return (
    <div className="bg-white shadow sm:rounded-lg p-6">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-medium text-gray-900">Expense Analytics</h2>
        <select
          value={period}
...
export default function ExpensesPage() {
  ...
  const [view, setView] = useState('list'); // 'list', 'analytics', 'categories'
  ...
  return (
    <div className="space-y-6">
      ...
      <Routes>
        <Route index element={
          view === 'analytics' ? <ExpenseAnalytics /> : <ExpenseListPage />
        } />
